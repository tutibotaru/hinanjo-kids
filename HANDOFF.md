...
- 運用方針 = **A1 完全自走**(本番 FKJ606 を **phase=1** で作成 → リーダーのフェーズ操作不要)
...
1. [ ] **主催者へ確認連絡文を送る**(文面は上の「セッション追記(2026-06-01)」内＋下記):
   ①公開イベント名 ②主催/協力表記 ③会場名 ④アプリ体験の時間枠 ⑤参加コード(FKJ606案) ⑥緊急連絡先4つ
2. [ ] **本番セッション FKJ606 を作成**(確定後)。⚠️ **必ず `phase=1`**（`docs/event-setup.md` のSQL参照）。
   production書き込み（※過去に Supabase の DELETE は安全ガードで拒否された経緯あり。実行時は要確認）
